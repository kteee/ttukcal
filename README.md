...
헤더 대메뉴는 **실수령 계산기**와 **생활 계산기**(드롭다운) 두 개다.

| 도구 | 경로 | 하는 일 |
| --- | --- | --- |
| 실수령 계산기 | `/salary` | 연봉 → 월 실수령액, 4대보험·소득세 내역, 연봉별 실수령액 표 |
| 날짜 계산기 | `/date` | 두 날짜 사이(일·주·년월일), 날짜 더하기·빼기 |
| 만나이 계산기 | `/age` | 만 나이 · 세는 나이 · 연 나이 |
...
- **URL 끝 슬래시**: GitHub Pages 는 `/age` 를 `/age/` 로 301 한다. 그래서
  canonical·사이트맵·내부 링크는 곧바로 200 을 받는 `/age/` 형태로 쓰고,
  조회 키는 끝 슬래시를 뗀 `/age` 로 통일한다([src/lib/path.ts](src/lib/path.ts)).
  둘이 어긋나면 JS 가 뜬 뒤 제목이 홈 기본값으로 덮여 검색엔진이 페이지를 홈으로 합친다.
...
[src/lib/salary.ts](src/lib/salary.ts) 가 2026년 9월 급여 기준으로 계산한다.

- 4대보험: 국민연금 4.75%(기준소득월액 41만~659만원, 매년 7월 조정),
  건강보험 3.595%, 장기요양 = 건강보험료 × 0.9448/7.19, 고용보험 0.9%
- 소득세: 근로소득 간이세액표 100%(소득세법 시행령 [별표 2], 2026. 2. 27. 개정).
  표는 공식 PDF 를 `pdftotext -layout` 으로 추출해
  [src/lib/income-tax-table.ts](src/lib/income-tax-table.ts) 로 만들었다.
  PDF 추출 시 빈칸 `-` 가 `--` 로 붙어 나오는 줄이 있어 한 칸씩 떼어 파싱했고,
  구간 연속성과 실제 세액 16개를 대조해 검증했다. 손으로 고치지 말고 다시 추출한다.
- 월급여 1,000만원 초과 계산식, 8~20세 자녀 공제(1명 20,830원, 2명 45,830원,
  3명부터 1명당 33,330원 추가), 11명 초과 가족 규칙도 별표 2 그대로 따른다.
- 원 단위 처리가 공식 문서에 없는 곳(국민연금·고용보험 보험료, 1,000만원 초과
  소득세)은 관행대로 10원 미만 절사. 부동소수점 오차(3,000,000 × 0.009 =
  26,999.99…)로 10원이 덜 잘리지 않게 절사 전에 아주 작은 값을 더한다.
- **해마다 갱신할 것**: 1월 요율, 7월 국민연금 상·하한, 간이세액표 개정 시 표 재추출.
